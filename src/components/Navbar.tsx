...
const Navbar: React.FC<NavbarProps> = (props) => {
  ...
  return (
    <div className="w-full flex flex-col">
      <nav className="navbar-container w-full border-b-1 p-8 flex-between space-x-2 border-b-[1px] border-gray_100">
              ...
              <>
                ...
                <li>
                  <Link to="/cart" className="text-lg relative">
                    ...
                    {count > 0 && (
                      <p className="bg-red-500 p-1 text-1 flex items-center justify-center text-white absolute rounded-full bottom-[-3px] right-[-3px] text-[8px] h-[13px] w-[13px]">
                        {count}
                      </p>
                    )}
                  </Link>
        ...
        <div className="hidden tablet:flex">
          {!isLoggedIn ? (
            <Link
              to="/login"
              className="flex gap-2 rounded-full bg-black p-3 px-6"
            >
        ...
        <div className="flex gap-10 items-center">
          ...
          {!isLoggedIn ? (
            <Link
              to="/login"
              className="flex gap-2 rounded-full bg-black p-3 px-6 tablet:hidden"
            >
              <p className="text-white text-sm">Log in</p>
            </Link>
          ) : (
        ...
        {isLoggedIn && burgerShown && (
          <div
            className="bg-black flex tablet:hidden z-10 absolute w-[80%] tablet:w-[60%] top-[10%] right-[15%] border border-gray_100 rounded-sm"
          ...
          >
            <ul className="w-full p-4 space-y-2">
              ...
              {(authRole === "seller" || authRole === "admin") && (
                <li className="rounded-sm hover:bg-white transition-all">
                  <Link
                    to={authRole === "seller" ? "/seller" : "/admin"}
                    className="text-lg"
                  >
                    <div className="flex-between justify-center space-x-1 p-2">
      ...
      {isProfileModalOpen && (
        <div
          className="absolute right-0 bg-gray shadow-lg rounded-lg p-4 m-12 z-50"
